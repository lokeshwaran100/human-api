...
const preference_learning_1 = require("../services/preference-learning");
...
async function decisionRoutes(fastify) {
    ...
    fastify.post('/api/v1/decisions', async (request, reply) => {
        try {
            const result = await (0, decision_engine_1.processDecisionRequest)(request.body);
    ...
    fastify.get('/api/v1/decisions/:request_id', async (request, reply) => {
        const { request_id } = request.params;
        const { data: req } = await (0, supabase_1.getSupabase)()
        ...
        const decision = await (0, audit_service_1.getDecisionByRequestId)(request_id);
        return reply.send({
    ...
    fastify.post('/api/v1/decisions/:request_id/approve', async (request, reply) => {
        const { request_id } = request.params;
        const { decision: userDecision } = request.body;
        // Get the original decision
        const originalDecision = await (0, audit_service_1.getDecisionByRequestId)(request_id);
        if (!originalDecision) {
        ...
        await (0, supabase_1.getSupabase)()
            .from('decision_requests')
            .update({
            status: shared_1.DecisionRequestStatus.COMPLETED,
            completed_at: new Date().toISOString(),
        })
            .eq('request_id', request_id);
        // Log the override
        if (originalDecision.id) {
            await (0, audit_service_1.logOverride)({
    ...
    fastify.post('/api/v1/decisions/:request_id/override', async (request, reply) => {
        const { request_id } = request.params;
        ...
        await (0, preference_learning_1.processOverride)(DEFAULT_USER_ID, request_id, originalDecision.decision, override_decision, reason, remember_preference);
        return reply.send({
            request_id,
    ...
    fastify.get('/api/v1/decisions', async (request, reply) => {
      ...
      `)
        ...
        const results = await Promise.all((data || []).map(async (req) => {
            const decision = await (0, audit_service_1.getDecisionByRequestId)(req.request_id);
            return {
                ...req,
